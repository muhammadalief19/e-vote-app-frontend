...
const SidebarAdmin = ({ hideSideBar }) => {
  const location = useLocation();
  const activeRoute = (route) => {
    let path = location.pathname;
    return path.split("/admin")[1].includes(route);
  };
  return (
    <>
      <div
        className={`absolute lg:relative lg:translate-x-0 w-[250px] h-full flex flex-col gap-2 items-center py-10 px-5 text-gray-100 border-r-1 transition-all transform duration-300 z-[9999] rounded-e-xl shadow-2xl shadow-blue-900 bg-slate-800 ${
        ...
        <div className="w-full py-7">
          <div className="flex h-max items-center gap-6">
            ...
            <p className="lg:text-lg md:text-base font-bold text-yellow-400">
              E-Vote
              <span className="text-blue-800"> HMPS</span>
            </p>
          </div>
        </div>
